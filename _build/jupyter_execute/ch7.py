...
# ```
# 
# 위 조건문이 실행되는 순간 변수 `x`가 가리키는 값이 0보다 작으면 `x는 음수다`를 출력하고, 그렇지 않으면 다음 명령문으로 넘어간다. 
...
# :label: cond_ex01  
# 
# 변수`score`에 임의의 정수를 할당한 다음 `score`가 80이상이면 `합격입니다.`를 출력하는 코드를 작성하여라. 
# 
# ```python
# >>> score = 95
# >>> if score >= 80 :
...
# :label: cond_ex02  
# 
# 변수 `n`에 임의의 정수를 할당한 다음 `n`이 홀수면 `n`의 제곱을 출력하는 코드를 작성하여라. 
# 
# ```python
# >>> n = 7
# >>> if n % 2 == 1 :
...
# 
# 
# 정수 `1`은 `True`라 아래와 같이 코드를 작성할 수도 있다.  
#  
# ```python
# >>> n = 7
# >>> if n % 2 :
...
# :class: info    
# 프로그래밍에서 코드 블록은 특정한 일을 수행하는 한 단위의 코드 묶음을 말한다. 보통 중괄호(`{}`)로 묶여 있는 경우가 많으나 파이썬에서는 들여쓰기로 코드 블록을 지정한다. 
# :::
# 
...
# :label: cond_ex03  
# 
# 변수 `x`에 임의의 정수를 할당한 다음 `x`가 0보다 작으면 `x는 음수`, 크거나 같으면 `x는 음수 아님`를 출력하는 코드를 작성하여라. 
# 
# ```python
# >>> x = -10
# >>> if x < 0 :
...
# `if-else` 조건문은 조건이 참이면 `코드1`을 실행하고, 거짓이면 `코드2`을 실행한다. 
...
# ```
# 
# 위 조건문은 조건을 한 번만 확인하기 때문에 {prf:ref}`cond_ex03`에서 `if`문을 두 번 사용한 코드보다 더 효율적으로 동작한다.    

# ````{prf:example}
# :label: cond_ex04  
# 
# 변수 `a`에 임의의 정수를 할당한 다음 `a`의 절댓값을 출력하는 코드를 작성하여라.  
# 
# ```python
# >>> a = -5
# >>> if a >= 0 :
...
# 
# ```python
# 조건이 참일 때 실행할 코드 if 조건 else 조건이 거짓일 때 실행할 코드
# ```
...
# `if-else` 조건문을 중첩해서 사용하면 원하는 대로 경우를 나누어서 처리할 수 있다. 예를 들어, `x`가 `y`보다 큰지 작은지 같은지를 출력하는 코드를 다음과 같이 작성할 수 있다. 
# 
# ```python
# if x < y :
#     print('x가 y보다 작다')
...
# :::{admonition} 참고    
# :class: info    
# 중첩 조건문을 사용하면 코드가 금방 복잡해질 수 있기 때문에 일반적으로 권장하지 않는다. 논리 연산자를 사용해 중첩 조건문의 사용을 피할 수 있다. 아래 코드는 중첩 조건문을 사용해 `x`가 0보다 크면서 동시에 10보다 작은 경우를 다룬다.  
# 
# ```python
# if x > 0 :
#     if x < 10 :
...
# ```
# 
# `print()` 함수가 `x > 0`와 `x < 10`을 모두 만족하는 경우에만 실행되기에 아래와 같이 `and` 연산자를 이용하면 조건문을 중첩해 사용할 필요가 없다.  
# ```python
# if x > 0 and x < 10 :
#     print('x는 0보다 크고 10보다 작다')
# ```
...
# :label: cond_ex05 
# 
# 변수`score`에 임의의 정수를 할당한 다음 `score`가 90이상이면 `A`, 80이상 90미만이면 `B`, 70이상 80미만이면 `C`, 60이상 70미만이면 `D`, 60미만은 `F`를 출력하는 코드를 작성하여라. 
# 
# ```python
# if score >= 90 :
#     print('A')
...
# ```
# 
# `score`가 90미만일 때만 그 다음 `elif` 조건을 확인하기 때문에 조건에 `< 90`은 생략할 수 있다.   
# 
# ```python
# if score >= 90 :
#     print('A')
...
# 아래 문제에서 값이 주어졌다고 하면 변수에 해당 자료형의 임의의 값을 할당한다. 

# ### 문제 
# 주어진 정수 `num1`이 홀수인 경우에만 그 값에 1을 더한 값을 출력하여라. 

# ### 문제  
# 주어진 정수 `num2`가 5보다 큰 경우에만 `num2`를 3으로 나누었을 때의 나머지를 출력하여라. 

# ### 문제  
# 주어진 정수 `num3`의 값에 따라 `양수입니다.`, `음수입니다.`, `0입니다.`를 출력하는 코드를 작성하여라. 

# ### 문제 
# 1종 운전면허를 취득하기 위해서는 학과시험에서 70점이상을 받아야 한다. 학과시험 점수가 주어졌을 때, 합격여부를 알려주는 코드를 작성하여라. 
...
# 
# 연도가 주어졌을 때, 윤년인지 아닌지를 출력하는 코드를 작성하여라. 

# ### 문제  
# 다음 코드의 결과값이 무엇인지 말하고, 코드를 실행시켜 확인하여라.  
# 
# ```python
# name = '강현'
# 
...
# ### 문제 
# 다음 코드에서 오류가 발생하는 이유를 모두 말하고, 이를 수정하여라.  
# 
# ```python 
# i = 10
# if i = 5 :
...
# ### 문제  
# 
# `input()` 함수로 입력 받은 정수가 짝수인지 홀수인지를 출력하는 코드를 작성하여라. 
# 

# ### 문제  
# `input()` 함수로 두 정수를 입력 받아 작은 수를 출력하는 코드를 작성하여라. 

# ### 문제  
# `input()` 함수로 세 정수를 입력 받아 가장 큰 수를 출력하는 코드를 작성하여라. 

# ### 문제 
# `input()` 함수로 세 정수를 입력 받아 처음 두 정수를 더한 값이 세 번째로 입력한 정수라면 `정답입니다`를, 아니면 `오답입니다`를 출력하는 코드를 작성하여라. 

# ### 문제  
# `input()` 함수로 입력 받은 정수가 주어진 정수 `answer`와 같으면 `정답입니다`를, 크면 `너무 커요` 작으면 `너무 작아요`를 출력하는 코드를 작성하여라. 

# ### 문제  
# `input()` 함수로 정수를 입력 받아 100보다 작거나 같으면 입력 받은 수를, 100보다 크면 100을 출력하는 코드를 작성하여라. 

# ### 문제  
# 골프는 낮은 점수가 이기고, 테니스는 높은 점수가 이기는 게임이다. 강현이와 나현이는 골프 또는 테니스 게임을 했다. 두 사람의 점수와 종목을 입력 받아 누가 이겼는지를 출력하는 코드를 작성하여라.   
...
# |151 ~ |매우나쁨|
# 
# `input()` 함수로 현재 미세먼지 농도를 입력 받아 예보 등급을 알려주는 코드를 작성하여라. 
# 

# ### 문제  
# 체질량지수<font size="2">Body Mass Inded, BMI</font>는 체중(kg)을 키(m)의 제곱으로 나눈 값으로 비만도 판정에 사용한다. 
# 
# $$BMI = \frac{몸무게}{키^2}$$  
# 
# * 단위 :
...
# |30이상|고도비만|
# 
# 키와 몸무게가 주어졌을 때, BMI와 체중 분류를 출력하는 코드를 작성하여라.
...
# 
# 주차시간을 입력 받아 주차요금을 출력하는 코드를 작성하여라. 

# ## [미니 프로젝트] 죄수의 딜레마

# 죄수의 딜레마<font size="2">Prisoner's Dilemma, PD</font>은 수학자 존 내시<font size="2">John Nash</font>가 고안한 게임으로 상황은 다음과 같다.   
# 
...
# 
# 용의자A가 되어 몇 년형을 받을지 확인하는 코드를 작성해보자.  
...
# * 용의자B의 선택은 `0`과 `1` 중 하나를 임의로 둔다. 아래 `random.randint(0, 1)`는 실행할 때마다 `0`과 `1` 중 하나를 임의로 반환한다.  
# ```python
# import random
...
# 컴퓨터와 가위바위보 게임을 한 다음 누가 이겼는지 출력하는 코드를 작성하여라.  
