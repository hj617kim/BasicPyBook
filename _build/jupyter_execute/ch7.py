...
# 중첩 조건문을 사용하면 많이 코드가 복잡해질 수 있기 때문에 가능하면 사용하지 않는 것이 좋다. 예를 들어, 아래 코드는 중첩 조건문을 사용해 `x`가 0보다 크면서 동시에 10보다 작은 경우를 다룬다.  
# 
# ```python
# if x > 0 :
#     if x < 10 :
...
# 
# * 특정 `조건`이 참일 때만 그 아래 들여쓰기된 `코드` 실행된다.  
# ```python
# if 조건 :
#     코드
# ```
...
# ### 문제   
# `input()` 함수로 두 분수 A, B의 분자와 분모에 해당하는 정수를 입력 받아 어느 분수가 더 큰지를 출력하는 코드를 작성하여라. 
